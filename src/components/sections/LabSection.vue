...
const typeClass: Record<LabItem['type'], string> = {
  ...
  experimento: 'border-emerald-500/40 bg-emerald-500/10 text-emerald-100',
...
<template>
  <section id="lab" class="scroll-mt-24 py-10 sm:py-10 lg:py-10 bg-slate-950/60 text-slate-100">
    <div class="max-w-5xl mx-auto px-4 space-y-6 sm:space-y-8">
      <header class="space-y-2">
        ...
        <p class="text-xs sm:text-sm md:text-base text-slate-400 max-w-2xl">
          Um espaço para concentrar o que estou estudando e testando: trilhas de HTML/CSS/JS,
          desafios com frameworks, projetos full stack e ajustes de firmware/3D printing.
        </p>
      </header>
      ...
      <div class="grid gap-4 sm:gap-5 md:grid-cols-3 text-xs sm:text-sm">
        ...
        >
          ...
          <div class="mt-3 flex flex-wrap gap-1.5 text-[10px] sm:text-[11px] text-slate-300">
            <span
          ...
          <div v-if="item.link" class="mt-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              class="inline-flex items-center gap-1 rounded-full border border-slate-600 px-3 py-1.5 text-[11px] font-medium text-slate-100 hover:border-emerald-400 hover:text-emerald-100 hover:bg-slate-900/70 transition"
            >
              {{ getLinkLabel(item) }}
            </a>
          </div>
